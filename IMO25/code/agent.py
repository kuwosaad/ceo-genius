...
# Config, logging, and utility imports with fallback
try:
    from .config import (
...
def update_scratchpad(
    scratchpad, new_fact=None, disproven_hypothesis=None, obstacle=None
):
    """
...
def lightweight_score_sketch(sketch: str, telemetry=None):
    ...
    if score == 0.0 and "parse error" in reason:
        print("[NRPA] DEBUG: Failed to parse viability score from verifier response:")
...
def init_explorations(
    ...
    if ENABLE_NRPA:
        strategy_selector = NRPAStrategySelector(
            api_client_funcs, STRATEGIST_MODEL_NAME
        )
    else:
        strategy_selector = SingleStrategySelector(
            api_client_funcs, STRATEGIST_MODEL_NAME
        )
    ...
    is_complete = check_if_solution_claimed_complete(
        solution
...
def agent(problem_statement, other_prompts=[]):
    ...
    init_result = init_explorations(
        problem_statement, True, other_prompts, backtracker, telemetry
    )
            ...
            if should_escalate:
                ...
                conversation_history_payload = {
                    ...
                    + [
                        ...
                        {
                            "role": "user",
                            "content": self_improvement_prompt.format(
                                scratchpad=scratchpad
                            ),
                        },
                    ],
